...
export async function runApprovalFlow(
  ...
  if (choice === "all") {
    for (const a of pending.filter((x) => x.type !== "tool_execute")) {
      tracker.updateStatus(a.id, "approved", true);
    }
    for (const s of pending.filter((x) => x.type === "tool_execute")) {
      const ok = await select({
        message: chalk.bold(`Execute shell command?\n${s.details.command}`),
        options: [
      ...
      if (isCancel(ok) || !ok) {
        tracker.updateStatus(s.id, "rejected", false);
      } else {
        tracker.updateStatus(s.id, "approved", true);
      }
    }
